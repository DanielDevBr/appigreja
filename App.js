...
// CERTIFIQUE-SE DE QUE O ARQUIVO firebaseConfig.js ESTÁ NA MESMA PASTA
...
function MembrosScreen({ navigation }) {
  ...
  const prepararEdicao = (item) => {
    setIdEdicao(item.id); setNome(item.nome); setCpf(item.cpf); setCargo(item.cargo); setAba('cadastro');
  };
  const excluirMembro = (id) => {
    Alert.alert("Excluir", "Tem certeza?", [{ text: "Cancelar", style: "cancel" }, { text: "Sim", onPress: () => removerMembro(id) }]);
  };
  ...
  return (
    <View style={styles.containerCinza}>
      ...
      {aba === 'cadastro' ? (
        ...
        <FlatList data={membros} keyExtractor={item => item.id} contentContainerStyle={{padding: 20}} renderItem={({item}) => (
...
function DizimosScreen({ navigation }) {
  ...
  const handleMudarValor = (texto) => {
    ...
    setValorNumerico(valor); setValorDisplay(valor.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }));
  };

  const handleSalvar = () => {
    if(!membroSel || valorNumerico <= 0 || !dataDisplay) return Alert.alert("Erro", "Preencha todos os campos");
    adicionarTransacao({ tipo: 'entrada', valorNumerico, valorFormatado: valorDisplay, data: dataDisplay, descricao: `${tipo} - ${membroSel.nome}` });
    Alert.alert("Sucesso", "Dízimo salvo na nuvem!"); navigation.goBack();
  };
    ...
    <View style={styles.containerCinza}>
      ...
      <ScrollView style={{padding: 20}}>
        ...
        <View style={{flexDirection: 'row', marginBottom: 15}}>
          {['Dízimo', 'Oferta'].map(op => (<TouchableOpacity key={op} onPress={() => setTipo(op)} style={[styles.radio, tipo===op && styles.radioAtivo]}><Text style={[styles.radioTxt, tipo===op && {color:'#fff'}]}>{op}</Text></TouchableOpacity>))}
        </View>
        <Text style={styles.label}>Data</Text>
        <TouchableOpacity style={styles.inputSelect} onPress={() => setModalData(true)}><Text style={{color: dataDisplay ? '#000' : '#aaa'}}>{dataDisplay || "Selecionar Data"}</Text></TouchableOpacity>
        <TouchableOpacity style={[styles.botaoPrincipal, {backgroundColor: '#2ecc71'}]} onPress={handleSalvar}><Text style={styles.textoBotao}>REGISTRAR ENTRADA</Text></TouchableOpacity>
      </ScrollView>
      <Modal visible={modalMembro} animationType="slide">
        <View style={{flex:1, padding: 20}}>
          <Text style={styles.tituloSecao}>Selecione o Membro</Text>
          <FlatList data={membros} keyExtractor={i=>i.id} renderItem={({item}) => (<TouchableOpacity style={styles.itemLista} onPress={()=>{setMembroSel(item); setModalMembro(false)}}><Text style={{fontWeight:'bold'}}>{item.nome}</Text></TouchableOpacity>)} ListEmptyComponent={<Text>Sem membros cadastrados.</Text>} />
...
function DespesasScreen({ navigation }) {
  ...
  const handleMudarValor = (texto) => {
    let numeroLimpo = texto.replace(/\D/g, ""); let valor = Number(numeroLimpo) / 100;
    setValorNumerico(valor); setValorDisplay(valor.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }));
  };
  const handleSalvar = () => {
    if(!descricao || valorNumerico <= 0 || !dataDisplay) return Alert.alert("Erro", "Preencha tudo");
    adicionarTransacao({ tipo: 'saida', valorNumerico, valorFormatado: valorDisplay, data: dataDisplay, descricao });
    Alert.alert("Sucesso", "Despesa salva na nuvem!"); navigation.goBack();
  };
  ...
  return (
    <View style={styles.containerCinza}>
      ...
      <ScrollView style={{padding: 20}}>
        ...
        <Text style={styles.label}>Data</Text>
        <TouchableOpacity style={styles.inputSelect} onPress={() => setModalData(true)}><Text style={{color: dataDisplay ? '#000' : '#aaa'}}>{dataDisplay || "Selecionar Data"}</Text></TouchableOpacity>
        <TouchableOpacity style={[styles.botaoPrincipal, {backgroundColor: '#e74c3c'}]} onPress={handleSalvar}><Text style={styles.textoBotao}>REGISTRAR SAÍDA</Text></TouchableOpacity>
      </ScrollView>
      <CalendarioModal visivel={modalData} fechar={()=>setModalData(false)} aoSelecionar={setDataDisplay} />
    </View>
  );
...
function RelatoriosScreen({ navigation }) {
  ...
  const handleExportar = async () => {
    ...
    try { await Share.share({ message: csvHeader + csvBody, title: "Relatorio_IgrejaEGD.csv" }); } catch (error) { Alert.alert("Erro", "Não foi possível compartilhar."); }
  };
  const entradas = transacoes.filter(t => t.tipo === 'entrada').reduce((acc, curr) => acc + (curr.valorNumerico||0), 0);
  const saidas = transacoes.filter(t => t.tipo === 'saida').reduce((acc, curr) => acc + (curr.valorNumerico||0), 0);
    ...
    <View style={styles.containerCinza}>
      ...
      <ScrollView style={{padding: 20}}>
        <View style={styles.tabelaContainer}>
          ...
          <View style={[styles.linhaTabela, {borderTopWidth: 1, marginTop: 5}]}><Text style={{fontWeight:'bold', fontSize:18}}>SALDO</Text><Text style={{fontWeight:'bold', fontSize:18, color: saldo>=0 ? 'blue' : 'red'}}>R$ {saldo.toFixed(2)}</Text></View>
        </View>
        <TouchableOpacity style={[styles.botaoPrincipal, {marginTop: 20, backgroundColor: '#107c41'}]} onPress={handleExportar}><Text style={styles.textoBotao}>EXPORTAR RELATÓRIO EXCEL</Text></TouchableOpacity>
      </ScrollView>
    </View>
  );
}
...
function ExtratoDetalhadoScreen({ route, navigation }) {
  ...
  return (
    <View style={styles.containerCinza}>
      ...
      <FlatList data={dadosFiltrados} keyExtractor={item => item.id} contentContainerStyle={{padding: 20}} renderItem={({item}) => (
          <View style={styles.itemLista}>
            <View><Text style={{fontWeight:'bold', color: '#333'}}>{item.descricao}</Text><Text style={{fontSize: 12, color:'#888'}}>{item.data}</Text></View>
            <Text style={{fontWeight:'bold', color: tipoFiltro==='entrada'?'green':'red'}}>{item.valorFormatado}</Text>
          </View>
...
export default function App() {
  ...
  useEffect(() => {
    const unsubMembros = onSnapshot(collection(db, "membros"), (snapshot) => { setMembros(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }))); });
    const unsubTransacoes = onSnapshot(collection(db, "transacoes"), (snapshot) => { setTransacoes(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }))); });
    return () => { unsubMembros(); unsubTransacoes(); };
  }, []);

  const adicionarMembro = async (dados) => { try { if (dados.id) { await updateDoc(doc(db, "membros", dados.id), { nome: dados.nome, cpf: dados.cpf, cargo: dados.cargo }); } else { await addDoc(collection(db, "membros"), { nome: dados.nome, cpf: dados.cpf, cargo: dados.cargo }); } } catch (e) { Alert.alert("Erro", "Falha ao salvar membro"); } };
  const removerMembro = async (id) => { try { await deleteDoc(doc(db, "membros", id)); } catch(e) { Alert.alert("Erro", "Falha ao excluir"); } };
  const adicionarTransacao = async (dados) => { try { await addDoc(collection(db, "transacoes"), dados); } catch(e) { Alert.alert("Erro", "Falha ao salvar transação"); } };
